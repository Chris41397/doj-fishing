...
RegisterNetEvent('fishing:server:catch', function() 
    ...
    if itemFound then
        for i = 1, itemCount, 1 do
			...
			elseif luck >= 35 and luck <= 40 then
				Player.Functions.AddItem('fishtacklebox', 1, nil, {["quality"] = 100})
				TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['fishtacklebox'], "add", 1)
				TriggerClientEvent('QBCore:Notify', src, "You found someones Tackel Box", "success")
...
RegisterNetEvent('doj-fishing:server:removekey', function() 
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.RemoveItem('fishingkey', 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["fishingkey"], "remove") 
end)

RegisterNetEvent('doj-fishing:server:treasurechest', function() 
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
